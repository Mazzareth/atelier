<div class="module-wrapper {{ $isEditMode ?? false ? 'draggable' : '' }}" data-id="{{ $module->id }}" data-type="text_block" data-settings="{{ json_encode($module->settings ?? []) }}">
    @if($isEditMode ?? false)
    ...
    <div style="background: var(--bg-panel); border: 1px solid var(--border-color); padding: 2.5rem; border-radius: 8px; border-left: 4px solid var(--accent-color);">
        <div class="module-content" style="color: var(--text-main); line-height: 1.6; white-space: pre-wrap;">
            @php
                $text = $module->settings['text'] ?? 'New text block.';
                
                // If it looks like Markdown, we render it; otherwise we just escape it
                $html = e($text);
